...
export default function InSpectrum({ type, onClose }) {
  return (
    <div className="InSpectrum">
      {type === "in" && <EmissionSpectrum onClose={onClose}/>}
      {type === "out" && <AbsorptionSpectrum onClose={onClose}/>}
    </div>
  );
}

function EmissionSpectrum({ onClose }){
  ...
  return(
    <div className="Spect-css-all">
      ...
      <p className="Spectrum-P">전자가 낮은 에너지 준위에서 높은 에너지 준위로 이동할 때,<br></br>해당 원소의 <span>
      흡수 스펙트럼</span> 이 나타납니다.</p>
      </div>
      ...
      <div className="Spectrum-Mean">
        ...
        <p className="SpectrumM-P">빛이 물질을 통과할 때 특정 파장의 빛이 흡수되어 그 부분이 검은 선이나 띠로 나타나는 현상입니다. 연속 스펙트럼을 가진 빛을 저온의 기체에 통과시키면, 기체 속 원자나 분자가 특정 파장의 에너지를 흡수하면서 발생합니다.</p>
      </div>
    </div>
  );
}

function AbsorptionSpectrum({ onClose }){
  const location = useLocation();
  ...
  return(
    <div className="Spect-css-all">
      ...
      <div className="Spectrum-H">
      <h2 className="Spectrum">{name} 스펙트럼</h2>
      <p className="Spectrum-P">전자가 높은 에너지 준위에서 낮은 에너지 준위로 이동할 때, <br></br>해당 원소의 <span>방출 스펙트럼</span>이 나타납니다.</p>
      </div>
        <div className="Spectrum-div">
          <Spectrum
            wavelengths={[]}
